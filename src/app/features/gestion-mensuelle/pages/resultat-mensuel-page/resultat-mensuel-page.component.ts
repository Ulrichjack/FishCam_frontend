...
export class ResultatMensuelPageComponent implements OnInit {
  ...
  async toggleBilanAnnuel(){this.bilanAnnuelOuvert.set(!this.bilanAnnuelOuvert());if(this.bilanAnnuelOuvert())await this.chargerAnnuel();}
  async changerAnnee(delta:number){const cible=this.anneeBilan()+delta;if(cible>new Date().getFullYear())return;this.anneeBilan.set(cible);await this.chargerAnnuel();}
  async chargerAnnuel(){this.isLoadingAnnuel.set(true);try{const r=await firstValueFrom(this.service.resultatAnnuel(this.anneeBilan()));this.resultatAnnuel.set(r.data);}catch{this.toast.error('Impossible de calculer le bilan annuel.');}finally{this.isLoadingAnnuel.set(false);}}
  ...
  ouvrir(p:'releve'|'charge'){if(p==='charge'&&this.panneau()==='charge'){this.annulerEdition();return;}this.panneau.set(p);}
